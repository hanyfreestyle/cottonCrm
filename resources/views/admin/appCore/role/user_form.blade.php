        ...
        <x-admin.card.def :page-data="$pageData">
            ...
            <form data-parsley-validate class="mainForm" action="{{route($PrefixRoute.'.update',intval($users->id))}}" method="post"
                ...
                <div class="col-lg-12">
                    ...
                    <hr>
                    <div class="row">
                        <x-admin.form.select-arr name="crm_crm" :sendvalue="old('crm_crm',$users->crm_crm)" :label="__('admin/config/roles.crm_crm')" col="3" select-type="selActive"/>
                        <x-admin.form.select-arr name="crm_sales" :sendvalue="old('crm_sales',$users->crm_sales)" :label="__('admin/config/roles.crm_sales')" col="3" select-type="selActive"/>
                        <x-admin.form.select-arr name="crm_tech" :sendvalue="old('crm_tech',$users->crm_tech)" :label="__('admin/config/roles.crm_tech')" col="3" select-type="selActive"/>
                    </div>
                    <div class="row">
                        <x-admin.form.select-multiple name="crm_team" :categories="$team" :sel-cat="old('crm_team',$users->crm_team)" :label="__('admin/config/roles.crm_team')"  :req="false" :has-trans="false" col="12"/>
                    </div>
                    <hr>
